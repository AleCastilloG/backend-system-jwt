// Esta función te da la fecha y hora actual

export class Datetime {
  getCurrentDateTime(dateSeparateSymbol: string = "-"): string {
    const dateTime: Date = new Date();
    let dateDay: string = this.formatWithTwoDigits(String(dateTime.getDate()));
    let month: string = this.formatWithTwoDigits(
      String(dateTime.getMonth() + 1)
    );

    let hour: string = this.formatWithTwoDigits(String(dateTime.getHours()));
    let minutes: string = this.formatWithTwoDigits(
      String(dateTime.getMinutes())
    );
    let seconds: string = this.formatWithTwoDigits(
  ...
  private formatWithTwoDigits(value: number | string): string {
    if (+value < 10) {
      return `0${value}`;
    }
    return String(value);
   ...
   */
  addDays(days: number, date: string, customDate: string = ""): string {
